  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex flex-wrap items-baseline gap-x-2.5 gap-y-1">
        {priceFrom && !priceMax ? (
          <span className="text-meta text-[12px] text-ink-400">Desde</span>
        ) : null}
        <span
          className={cn(
            'text-subheading',
        ...
        {onSale ? (
          <span className="text-body text-[13px] text-ink-400 line-through">
            {formatCurrency(compareAtPrice!)}
          </span>
        ) : null}
      </div>
      {showSavings && onSale ? (
        <p className="text-offer-gradient text-ui text-[12px]">
