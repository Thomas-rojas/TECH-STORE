...
export function Header() {
  ...
  return (
    <header
      className={cn(
        'fixed inset-x-0 top-0 z-50 border-b text-ink-800 transition-all duration-300 ease-out',
        scrolled || openSlug
          ? 'border-brand-300/25 bg-surface/90 shadow-[0_10px_36px_rgb(102_201_207_/_0.12)] backdrop-blur-xl'
          : 'border-brand-300/15 bg-surface/80 backdrop-blur-xl',
      )}
    >
      <button
        type="button"
            ...
            {NAV_DEPARTMENTS.map((department) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    role="menu"
                    className={cn('nav-flyout', isOpen && 'is-open')}
                  >
                    {department.items.map((entry, index) => {
                      const isLast = index === department.items.length - 1
                      const isFirst = index === 0
                      return (
                        <div key={entry.href}>
                          {isLast ? <div className="nav-flyout-divider" role="separator" /> : null}
                          <Link
                            to={entry.href}
                            role="menuitem"
                            ...
                            className={cn(
                              'nav-flyout-item',
                              isLast && 'nav-flyout-item--cta',
                            )}
                          >
                            <span className="nav-flyout-item__dot" aria-hidden />
                            <span className="nav-flyout-item__label">{entry.label}</span>
                            {isFirst ? (
                              <span className="nav-flyout-item__badge">Nuevo</span>
                            ) : null}
                          </Link>
                        </div>
                      )
